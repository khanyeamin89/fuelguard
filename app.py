...
def show_instruction():
    ...
    * **🚜 কৃষক (Farmer):** নিবন্ধনের জন্য **NID** এবং **UNO সার্টিফিকেট** নম্বর প্রয়োজন। সেচ কাজের প্রয়োজনে আপনাদের জন্য রিফিলের সময়ের কোনো সীমাবদ্ধতা নেই।
    ...
    * **৭২ ঘণ্টা নিয়ম:** সাধারণ রাইডাররা একবার তেল নেওয়ার পর পরবর্তী **৭২ ঘণ্টা** পর্যন্ত পুনরায় তেল নিতে পারবেন না।
    ...
    #### **৩. অপারেটরদের জন্য (Pump Operators)**
    * অপারেটরদের দৈনিক **সিক্রেট পিন** ব্যবহার করে লগইন করতে হবে।
    * তেল প্রদানের সময় ছবি তোলা **ঐচ্ছিক**, তবে তথ্যের সত্যতা যাচাই করা জরুরি।
...
if st.session_state.app_mode in ["Farmer", "Govt", "General"]:
    ...
    with tab1:
        s_id = st.text_input("আইডি বা গাড়ির নাম্বার দিয়ে সার্চ করুন")
        if s_id:
            user = get_user_by_id(s_id)
            if user:
                # নাম সুন্দরভাবে Sentence Case-এ দেখানো হচ্ছে
    ...
    with tab2:
        with st.form("reg_form"):
            ...
            if mode in ["General", "Govt"]:
                ...
                v_num = c_n.text_input("গাড়ির নাম্বার (যেমন: 12-3456)", key=f"n_{mode}")
...
# --- ৬. পাম্প অপারেটর প্যানেল ---
    ...
    if not st.session_state.pump_auth:
        ...
        pin = st.text_input("আজকের সিক্রেট পিন দিন", type="password")
        c1, c2 = st.columns(2)
        with c1:
        ...
        p_search = st.text_input("আইডি বা গাড়ির নাম্বার দিয়ে সার্চ করুন")
        if p_search:
            user = get_user_by_id(p_search)
            if user:
                # নাম ফরম্যাট করে দেখানো
                ...
                if eligible:
                    ...
                    if st.button("💾 ডাটা সেভ করুন", use_container_width=True, type="primary"):
                        ...
                        if photo:
                            ...
                            except: st.warning("ছবি আপলোড করা সম্ভব হয়নি।")

                        supabase.table("riders").update(update_vals).eq("rider_id", user['rider_id']).execute()
                        st.success("তথ্য সফলভাবে সংরক্ষিত হয়েছে!"); st.rerun()
                else: st.error("🚫 এই ইউজার বর্তমানে লকড (৭২ ঘণ্টা নিয়ম)")
            else: st.error("দুঃখিত, এই আইডিটি ডাটাবেজে পাওয়া যায়নি।")

st.markdown("---")
st.caption("* বিশেষ ছাড়: কৃষক এবং সরকারি জরুরি সেবার গাড়ির ক্ষেত্রে রিফিলের সময়ের সীমাবদ্ধতা নেই।")
